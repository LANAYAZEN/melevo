...
Class Action {
	...
	function signup(){
		...
		if($chk > 0){
			return json_encode(array('status'=>2, 'msg'=>'Email already exists'));
			exit;
		}
		...
		if($save){
			$login = $this->login2();
			return json_encode(array('status'=>1, 'msg'=>'Signup successful'));
		} else {
			return json_encode(array('status'=>0, 'msg'=>'Signup failed'));
		}
	}
